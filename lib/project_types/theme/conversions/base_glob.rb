      ...
      class << self
        ...
        def convert(parser)
          ...
          option_indexes(argv).each do |option_index|
            values += convert_option(argv, parser, option_index)
          end
        ...
        def convert_option(argv, parser, option_index)
        ...
        def option_indexes(argv)
          argv.each_with_index.select { |item, _index| options.include?(item) }.map { |pair| pair[1] }
        end
